...
<div class="product productitem_<?php print $product->product_id?>">
        ...
        <?php if (is_array($product->extra_field)){?>
            <div class="extra_fields">
                <?php foreach($product->extra_field as $extra_field){?>
                    <div>
                        <span class="label-name" data-name="<?php print $extra_field['name'];?>">
    ...
    <a href="/order-category?product=<?php echo urlencode(base64_encode($product->name)) ?>" class="back-coll cbox iframe boxw_300 boxh_530" class="cart-wrap">
        <span class="cart-bg">        
        </span>
    </a>
</div>
